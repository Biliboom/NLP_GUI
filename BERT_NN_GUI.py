...
import torch                     #Importing pytorch. While this project uses pytorch for all ML purposes, one could also use tensorflow
import torch.nn as nn            

from transformers import BertTokenizer  
from transformers import BertModel

from cleaning_for_GUI import convert      #function that turns data file into pandas dataframe
from cleaning_for_GUI import splitter     #function for splitting data into test,train and validation data:     returns  X_test, y_test, X_train, y_train, X_val, y_val
...
def bert_nn(filepath, number_of_epochs = 2, test=False):

    #Notice that here we only have filepath as an input. This is due to how the gradio gr.UploadButton method works. It returns the entire filepath including the filename. As such, there is no need to have two seperate parameters for location and filename
    df = convert(filepath)                                      #Definining dataframe where all training and validation data will be pulled from
    ...
    filename = os.path.basename(os.path.normpath(f"{filepath}"))          #Takes filepath and turns in into a string for just the filename
    savename = filename.replace(".csv","")                                #Replaces .csv ending with .pickle
    with open(f'Your_Pickle/Neural Net/{savename}.pickle', 'wb') as handle:
        pickle.dump(bert_classifier, handle)                              #Save the model to Your_Pickle folder (pickle folder for user uploaded datafiles)
